    ...
    <div class="font-sans antialiased shadow pin-t">
        <nav class="flex items-center justify-between flex-wrap bg-grey-darkest text-grey-lightest p-5 h-auto">
            ...
            <div class="block sm:hidden">
                <button @click.prevent="toggle" class="flex items-center px-3 py-2 border rounded text-teal-lighter border-teal-light hover:text-white hover:border-white">
            ...
            <div :class="open ? 'block': 'hidden'" class="w-full flex-grow sm:flex sm:items-center sm:w-auto my-auto">
                <div class="text-sm sm:flex-grow">
                    <nuxt-link to="/portfolio" class="no-underline block text-xl mt-4 sm:inline-block sm:mt-0 text-teal-lighter hover:text-white mr-4">
                        Portfolio
                    </nuxt-link>
                    <nuxt-link to="/stocks" class="no-underline block text-xl mt-4 sm:inline-block sm:mt-0 text-teal-lighter hover:text-white mr-4">
                        Stocks
                    </nuxt-link>
                </div>
                <div class="flex pt-1">
                    <div class="relative group" @click.prevent="toggleNav">
                        <div class="flex cursor-pointer md:mx-4 text-xl text-teal-lighter hover:text-white">
                            Save/Load
                            <svg class="fill-current h-6 mt-1 w-4" xmlns="http://www.w3.org/2000/svg" ><path d="M9.293 12.95l.707.707L15.657 8l-1.414-1.414L10 10.828 5.757 6.586 4.343 8z"/></svg>
                        </div>
                        <div class="items-center absolute border border-t-0 rounded-b-lg group-hover:visible w-full" :class="dialog ? 'block': 'hidden'">
                            <a href="#" class="no-underline text-xl bg-white text-center px-4 py-2 block text-teal hover:bg-teal-dark hover:teal-white ">Save</a>
                            <a href="#" class="no-underline text-xl bg-white text-center px-4 py-2 block text-teal hover:bg-teal-dark hover:teal-white ">Load</a>
                        </div>
                    </div>                   
                </div>
